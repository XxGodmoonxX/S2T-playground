  ...
  ws.on('message', async (message: Buffer | string) => {
    ...
    if (typeof message === 'string' || message.toString().startsWith('{')) {
      ...
      if (client && client.openaiWs && client.openaiWs.readyState === WebSocket.OPEN) {
        ...
        if (client) {
          client.audioChunks.push(Buffer.from(message));
        }
      }
    }
  });
...
// OpenAI WebSocket接続の初期化
async function initializeOpenAIWebSocket(clientId: string, apiKey: string, model: string): Promise<void> {
  if (!apiKey) {
  ...
  openaiWs.on('message', (data: Buffer | ArrayBuffer | Buffer[]) => {
    try {
      // OpenAIからのレスポンスを解析
      let dataStr: string;
      if (Buffer.isBuffer(data)) {
        ...
        // Buffer配列の場合は連結してから文字列に変換
        const combinedBuffer = Buffer.concat(data);
        dataStr = combinedBuffer.toString('utf-8');
      } else {
        // 文字列として扱う
        dataStr = String(data);
      }
      ...
      // データの型に応じて適切な表示形式を選択
      let messagePreview = '(データ形式不明)';
      try {
        if (Buffer.isBuffer(data)) {
          messagePreview = `(バイナリデータ: ${data.length} バイト)`;
        } else if (data instanceof ArrayBuffer) {
          messagePreview = `(ArrayBuffer: ${data.byteLength} バイト)`;
        } else if (Array.isArray(data)) {
          messagePreview = `(Buffer配列: ${data.length}個のバッファ)`;
        } else {
          messagePreview = `(データタイプ: ${typeof data})`;
        }
      } catch (previewError) {
        messagePreview = '(データのプレビュー作成中にエラーが発生)';
      }
      
      console.error('元のメッセージ:', messagePreview);
    }
  });
